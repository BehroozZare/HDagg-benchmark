...
namespace sym_lib
{
	...
	CSR *csc_to_csr(CSC *A)
	{
		...
		size_t nrow = B->m;
		int counter = 0;
		//Doing an scan and again a size_t can solve this castingproblem
		...
		for (int i = 0; i < (int)ncol; i++)
		{
			...
			{
				int row = A->i[j];
				int index = rowptr[row] + rowCnt[row]; // or rowpr[row + 1]
				colind[index] = i;
				if (!B->is_pattern)
					values[index] = A->x[j];
				rowCnt[row]++; // it should be something like index++, so it makes the code a bit unclear
			}
		}
		delete[] rowCnt;
		return B;
	...
	CSC *make_half(size_t An, int *Ap, int *Ai, double *Ax, bool lower)
	{
		size_t Bn = An;
		size_t Bnz = 0;
		// Counting the number of none-zeros un half of the matrix based on lower flag
		// Also using for(size_t i, ..) can eleminate casting on An Behrooz
    ...
    std::cout<<"\n";
		...
		int curPart = 0;
		outSize = 0;
		for (int curNode = 0; curNode < inSize; ++curNode)
		{ //k is in partitioned node
			if (inTree[curNode] == -2)
				continue;
			if (!visited[curNode])
			{
							...
							if (!visited[tmpChild])
								...
								{ //First add leaves
									...
									if (!visited[k])
									{
										...
										if (stack.size() > 0)
										{
											while (tmpk != stack[stack.size() - 1])
											{					   //Mark all parents upwards, they should be in the partiotion
