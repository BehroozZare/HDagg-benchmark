...
namespace sym_lib
{
    ...
    int buildLevelSet_CSC_Naive(size_t n, const int* Lp, const int* Li,
        ...
        std::vector<int> nodeList1(n);
        std::vector<int> nodeList2(n);
        int cnt = 0;
        for(auto& iter: nodeList1){
            iter = cnt++;
        }

        int nlevels = 0;
        int* curr_node_list = nodeList1.data();
        int* next_node_list = nodeList2.data();
        levelPtr.push_back(0);
        int total_node_visited = 0;
        int curr_num_nodes = n;
        int next_num_nodes = 0;
        while(curr_num_nodes != 0){
            for(int i = 0; i < curr_num_nodes; i++){
                int node = curr_node_list[i];
                if(inDegree[node] == 1){
                    levelSet[total_node_visited++] = node;
                } else {
                    next_node_list[next_num_nodes++] = node;
                }
            }

            //Update the buffers and their counters for next iteration
            int* tmp = curr_node_list;
            ...
            levelPtr.push_back(total_node_visited);
            for(int node_ptr = levelPtr[nlevels]; node_ptr < levelPtr[nlevels + 1]; node_ptr++){
                int node = levelSet[node_ptr];
                for (int child_ptr = Lp[node]; child_ptr < Lp[node + 1]; ++child_ptr){
                    int child = Li[child_ptr];
                    if(child != node){//For efficiency, we can skip diagonal edges
                        inDegree[child]--; //removing corresponding edges
                    }
                }
            }
            nlevels++;
							...
							int *&levelPtr, size_t *&levelSet)
	{
		...
		for (size_t s = 1;
			 s <= blockNo; ++s)
		{ //In degree computation in assembly tree
	...
	int level_set_bn(int n, const int *Lp, const int *Li, int *&levelptr, int *&levels,
	...
	int find(const int *supernodes, int start, int end, int target)
	{
		...
		if (end - start == 1)
		{
			if (supernodes[start] == target)
				return start;
			else if (supernodes[end] == target)
				return end;
		}
...
*/
	...
	compute_depth(CSC *A, int *degrees)
	{
		...
		//print_csc("AT\n",AT->n,AT->p,AT->i,AT->x);
		level_num = build_levelSet_CSC(A->n, AT->p, AT->i, level_ptr, level_set, nullptr);
