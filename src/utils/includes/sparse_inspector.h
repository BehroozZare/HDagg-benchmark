...
/// Creates levelset from a CSC matrix that is we now the connection from child to ancestor,
/// the matrix should form DAG i.e., no cycle. It returns -1 if
/// the input graph has a cycle.
/// \param n Number of Nodes or columns
/// \param Lp As for DAG it is vertex pointer and Column pointer for CSC format
/// \param Li As for DAG it is Edge index or row indices for CSC format
/// \return levelPtr it shows the nodes in each level lvl for example: range [levelPtr[lvl], levelPtr[lvl + 1]) in levelSet
/// \return levelSet it stores the nodes
