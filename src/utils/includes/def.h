...
struct CSC
{
  ...
  CSC(size_t M, size_t N, size_t NNZ, bool ip) : m(M), n(N), nnz(NNZ), is_pattern(ip)
  {
    is_pattern = ip;
    pre_alloc = false;
    if (N > 0)
...
struct CSR
{
  ...
  CSR(size_t M, size_t N, size_t NNZ, bool ip) : m(M), n(N), nnz(NNZ), is_pattern(ip)
  {
    is_pattern = ip;
    pre_alloc = false;
    if (M > 0)
