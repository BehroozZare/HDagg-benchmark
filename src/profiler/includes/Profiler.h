...
namespace sym_lib{
 ...
 protected:
     virtual FusionDemo *construct_demo(CSR *L, CSC *L_csc,
                                        double *correct_x, std::string name,
                                        PAPIWrapper *pw) {};

     virtual FusionDemo *construct_demo(CSR *L, CSC *L_csc,
                                        double *correct_x, std::string name,
                                        PAPIWrapper *pw, int num_threads) {};


     virtual FusionDemo *construct_demo(CSR *L, CSC *L_csc,
                                        double *correct_x, std::string name,
                                        PAPIWrapper *pw, int num_threads, int p2, int p3) {};
  ...
  virtual FusionDemo *construct_demo(CSR *L, CSC* L_csc, CSC *Lt_csc,
    CSR *A, CSC *A_csc,
    double *correct_x, std::string name,
    PAPIWrapper *pw, int p1, int p2, int p3){};/*{
 ...
 public:
  ...
  void profile(CSR *L, CSC *L_csc, double *correct_x, std::string name, int p1)
  {
      ...
      for (int i = 0; i < num_iter; ++i) {
          ...
          auto *fd = construct_demo(L, L_csc, correct_x, name, pw, p1);
...
//          omp_set_num_threads(p1);
     ...
     void profile(CSR *L, CSC *L_csc, double *correct_x, std::string name, int p1, int p2, int p3)
     {
         int num_iter = ceil(event_codes_.size() / (double)bundle_size_) ;
         for (int i = 0; i < num_iter; ++i) {
             std::vector<int> tmp;
             ...
             auto *pw = new PAPIWrapper(tmp, num_instances_);
             auto *fd = construct_demo(L, L_csc, correct_x, name, pw, p1, p2, p3);
             fd->set_num_threads(p1);
//          omp_set_num_threads(p1);
             fd->evaluate();
             counter_bundles_.insert(counter_bundles_.end(),pw->counter_bundles_.begin(),
     ...
     void profile(CSR *L, CSC *L_csc, double *correct_x, std::string name, int p1, int p2, int p3, bool flag)
     {
         int num_iter = ceil(event_codes_.size() / (double)bundle_size_) ;
         for (int i = 0; i < num_iter; ++i) {
             std::vector<int> tmp;
             ...
             auto *pw = new PAPIWrapper(tmp, num_instances_);
             auto *fd = construct_demo(L, L_csc, correct_x, name, pw, p1, p2, p3, flag);
             fd->set_num_threads(p1);
//          omp_set_num_threads(p1);
             fd->evaluate();
             counter_bundles_.insert(counter_bundles_.end(),pw->counter_bundles_.begin(),
