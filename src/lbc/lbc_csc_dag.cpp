...
int get_coarse_Level_set_DAG_CSC03_parallel(
        ...
        int divRate, int numThreads, double *nodeCost, bool binPacking = true){
 ...
 int averageCC = 0;
 int levelNo = build_levelSet_CSC(n, lC, lR, levelPtr, levelSet, nullptr);
...
//    int *partition2Level = new int[levelNo + 1]();
    ...
    int lClusterCnt = height_partitioning_DAG_trng(
            levelNo, levelPtr, NULL, originalHeight, innerParts, minLevelDist, divRate,
            innerPartsSize, slackGroups, NULL, partition2Level.data());
...
//    delete[] partition2Level;
    ...
    int skeletonCoarsening(
        ...
        std::vector<int> remainingNodes, remainingTmp;
        std::vector<bool> isChanged(n_nodes, false);
        int originalHeight = 0;
        finaLevelPtr.resize(n_nodes + 1);
        finalPartPtr.resize(n_nodes + 1);
        finalNodePtr.resize(n_nodes);
        finaLevelPtr[0] = 0;
        ...
        auto innerParts = numThreads;
        std::vector<double> nodeCost (n_nodes, 1);
        std::vector<int> innerPartsSize(WM.size(), numThreads);
        int lClusterCnt = innerPartsSize.size() - 1;
        std::vector<int> partition2Level(WM.begin(), WM.end());

        make_partitions_parallel(n_nodes, DAG_ptr, DAG_set, finaLevelPtr.data(), finalNodePtr.data(), finalPartPtr.data(),
                                 innerParts, originalHeight, nodeCost.data(), node2Level,
                                 innerPartsSize, lClusterCnt, partition2Level.data(),
                                 levelPtr, levelSet, numThreads, false);

        finaLevelNo = lClusterCnt;
        if (true) { // Verification of the set.
            std::vector<bool> checkExist(n_nodes, false);
            for (int i = 0; i < lClusterCnt; ++i) {
                for (int k = finaLevelPtr[i]; k < finaLevelPtr[i + 1]; ++k) {
                    for (int j = finalPartPtr[k]; j < finalPartPtr[k + 1]; ++j) {
                        assert(checkExist[finalNodePtr[j]] == false);
